...
def using_llm(prompt: str) -> ProtocolSequences:
    ...
    try:
        ...
        index = 0
        os.makedirs(os.path.join(LLM_RESULT_DIR, "4_repetited_message_sequences"), exist_ok=True)
        while os.path.exists(os.path.join(LLM_RESULT_DIR, "4_repetited_message_sequences", f"response_{index}.json")):
            index += 1
        protocol_file = os.path.join(LLM_RESULT_DIR, "4_repetited_message_sequences", f"response_{index}.json")
